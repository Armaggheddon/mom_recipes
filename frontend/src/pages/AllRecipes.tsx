...
export default function AllRecipes() {
	...
	async function handleOnSearch(query: string, typeFilters: { [key: string]: string }) {
		...
		const filteredRecipes = allRecipes.filter(recipe => {
			...
			const matchesType = Object.values(typeFilters).length === 0 || (recipe.type && Object.values(typeFilters).includes(recipe.type));
	...
	return (
		<div>
			...
			<div>
				{(isQueryResult && recipes.length > 0 && (recipeQuery || Object.keys(typeFilters).length > 0)) && (
					<div className="flex flex-row items-center mb-4 space-x-2">
						<div
							className="flex flex-row align-middle flex-wrap ml-8"
						>
							<div className="text-on-surface">
								Found {recipes.length} recipe{recipes.length === 1 ? '' : 's'} for <span className="font-bold">{buildSearchDescription()}</span>
							</div>
						</div>
						{(isQueryResult) && (
							refreshButton()
