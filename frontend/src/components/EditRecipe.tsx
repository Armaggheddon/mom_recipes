  ...
  return (
    <div className="flex flex-col md:flex-row md:space-x-4">
      ...
      >
        ...
        <div>
          <label className="text-primary block mb-2 font-semibold">
            Recipe Name:
          </label>
          <input
            type="text"
          ...
          <input
            ...
            onChange={(e) =>
              setEditedRecipe({
                ...editedRecipe,
                time_to_cook: parseInt(e.target.value),
              })
            }
            className="w-full p-2 border rounded mb-4 bg-background text-on-background"
          />
            ...
            <div className="rounded-full bg-primary text-on-primary px-3 py-1 inline-block">
              <button
                onClick={() =>
                  setEditedRecipe({
                    ...editedRecipe,
                    servings: Math.max(0, editedRecipe.servings - 1),
                  })
                }
              >
                -
              </button>
              {editedRecipe.servings === undefined ||
              editedRecipe.servings === 0 ? (
                <span className="mx-2">N/A</span>
              ) : (
                <span className="mx-2">
                  {editedRecipe.servings} serving
                  {editedRecipe.servings !== 1 ? "s" : ""}
                </span>
              )}
              <button
                onClick={() =>
          ...
          {editedRecipe.steps.map((step, index) => {
            return (
              <div key={index} className="flex space-x-2 mb-4 items-center">
                <span className="text-on-surface font-bold text-lg font-display me-2">
                  {index + 1}
                </span>
                <textarea
        ...
        <div className="flex justify-center space-x-4 mt-6">
          ...
          <IconButton
            icon="save"
            onClick={() => onEditSave(editedRecipe)}
            label="Save"
          />
        </div>
      </div>
