  ...
  const validate = useCallback((r: Recipe): ValidationErrors => {
    ...
    if (r.nutrition) {
      ...
      if (
        ...
        (carbohydrates < 0 || carbohydrates > 2000)
      )
    ...
    r.ingredients.forEach((ing, idx) => {
      if (!ing.name.trim())
        e[`ingredients.${idx}.name`] = "Ingredient name required.";
      if (!ing.quantity.trim())
        e[`ingredients.${idx}.quantity`] = "Quantity required.";
    ...
    r.steps.forEach((step, idx) => {
      if (!step.description.trim())
        e[`steps.${idx}.description`] = "Step cannot be empty.";
    ...
    <div className="flex flex-col md:flex-row md:space-x-6">
      ...
      >
        ...
        <fieldset className="border border-outline rounded-md p-4 bg-surface-container">
          ...
          <div className="mb-4">
            <label
              className="text-primary block mb-1 font-semibold"
              htmlFor="recipeName"
            >
              Recipe Name
            </label>
            ...
            {[
              ...
              const value =
                editedRecipe.nutrition?.[
                  n.key as keyof typeof editedRecipe.nutrition
                ] ?? "";
              return (
                <div key={n.key} className="flex flex-col">
                  ...
                  />
                  {errors[fieldKey] && touched[fieldKey] && (
                    <span className="text-error text-2xs mt-1">
                      {errors[fieldKey]}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="min-w-[72px] text-center text-sm font-medium">
                  {editedRecipe.servings
                    ? `${editedRecipe.servings} ${editedRecipe.servings === 1 ? "serving" : "servings"}`
                    : "N/A"}
                </span>
                <button
                  type="button"
            ...
            <div className="flex flex-col col-span-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setEditedRecipe({
                    ...editedRecipe,
                    type: e.target.value as Recipe["type"],
                  })
                }
        ...
        <div className="flex justify-center space-x-4 mt-4">
          ...
          <div
            className={`relative ${!canSave ? "opacity-50 pointer-events-none" : ""}`}
          >
            <IconButton
              icon="save"
              onClick={attemptSave}
