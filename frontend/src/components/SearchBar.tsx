...
export default function SearchBar({ onSearch }: SearchBarProps) {
	...
	const recipeTypes = ['dessert', 'main_dish', 'side_dish', 'appetizer', 'beverage'];
	...
	return (
		<div className="w-full mx-4 p-4">
			<div className=" flex flex-row space-x-4">
				...
				/>
				<button className="bg-primary text-on-primary h-14 w-14 rounded-full hover:opacity-70"
					onClick={() => {
						onSearch(query, typeFilters);
						resetSearchParams();
					}}>
					<span className="material-symbols-rounded scale-110">search</span>
				</button>
				<button className="bg-surface-variant text-on-surface-variant h-14 w-14 rounded-full hover:opacity-70"
					onClick={() => {
						setShowFilters(!showFilters);
					}}>
					<span className="material-symbols-rounded scale-110">filter_list</span>
				</button>
			</div>
					...
					<div className="flex flex-wrap py-2">
						{
							recipeTypes.map((type) => (
								...
								>
									...
									{typeFilters[type] && (<span className="material-symbols-rounded align-middle me-2">check</span>)}
