...
const RecipeApi = {
    ...
    getRecipes: async (currentPage: number = 0, recipesPerPage: number = 10, filterBy: { type?: string; queryString?: string } = {}): Promise<RecipeTiny[]> => {
        let queryParams = `?start=${currentPage * recipesPerPage}&limit=${recipesPerPage}`;

        if (filterBy.type || filterBy.type === '') {
            queryParams += `&typeFilter=${encodeURIComponent(filterBy.type)}`;
        }
    ...
    postNewRecipe: async (images: File[]): Promise<number | null> => {
        try {
            ...
            if (!response.ok) {
                ...
                console.error("Failed to post new recipe, status:", response.status, errorData);

                // Handle validation errors (non-recipe images)
                if (response.status === 400 && errorData.type === 'validation_error') {
                    throw new Error(errorData.error || "The uploaded images do not appear to contain a valid recipe.");
                }

                // Handle other errors
                throw new Error(errorData.error || "Failed to create recipe. Please try again.");
            }
    ...
    updateRecipe: async (id: number, updatedData: Partial<Recipe>): Promise<Recipe> => {
        try {
            const response = await fetch(`${API_URL}/${id}`, {
                method: 'PUT',
                headers: {
