...
import { Pool } from "pg";
import { PG_USER, PG_HOST, PG_DATABASE, PG_PASSWORD, PG_PORT } from "../config";
...
export const RecipeData = {
  ...
  getRecipePage: async (
    start: number,
    limit: number,
    filterBy: { type?: RecipeType[]; queryString?: string },
  ): Promise<RecipeTiny[]> => {
    const client = await pool.connect();
    try {
  ...
  updateRecipe: async (
    id: number,
    recipeUpdate: Partial<Omit<Recipe, "id">>,
  ): Promise<Recipe | null> => {
    const client = await pool.connect();
    try {
      ...
      const values = [
        ...
        recipeUpdate.ingredients
          ? JSON.stringify(recipeUpdate.ingredients)
          : null,
...
// Export a cleanup function to close the pool (useful for tests)
