...
describe("Recipe Controller", () => {
  ...
  it("GET /api/v1/recipes/latest - should return latest recipes", async () => {
    const mockLatest = [
      {
        id: 2,
        name: "Latest Recipe",
        description: "Newest recipe.",
        image_path: "path/to/latest.jpg",
        time_to_cook: 15,
        type: "dessert" as RecipeType,
      },
    ];
    jest.mocked(RecipeService).getLatestRecipes.mockResolvedValue(mockLatest);

    const response = await request(app).get("/api/v1/recipes/latest");
    expect(response.status).toBe(200);
    expect(response.body).toEqual(mockLatest);
    expect(RecipeService.getLatestRecipes).toHaveBeenCalled();
  });

  it("GET /api/v1/recipes/abc - should 404 due to regex constraint", async () => {
    const response = await request(app).get("/api/v1/recipes/abc");
